...
const CreateTicket = () => {
  ...
  const [endSaleDate, setEndSaleDate] = useState(moment().format('DD-MM-YYYY'));
  ...
  const onPressCreate = () => {
    const timezone = moment(event.time)
      .format('Z')
      ?.replace('0', '')
      ?.replace(':', '')
      ?.replace('0', '')
    ...
    if (!nameTicket.length) {
      setErrorName(true);
    } else if (!priceTicket.length) {
      setErrorPrice(true);
    } else if (!quantityTicket?.length) {
  ...
  // 12 часов старт продаж 23 59 - конец продажи билета
      ...
      <RN.View style={{marginHorizontal: 4}}>
        ...
        <RN.View>
          <Input
            ...
            isErrorBorder={errorPrice}
            // onFocusInput={() => setErrorPrice(false)}
          />
      ...
      <RN.View style={styles.saleDatesContainer}>
        <RN.View>
          ...
          <RN.TouchableOpacity
            ...
            <RN.Text style={styles.saleDatesText}>{`${moment(
              startSaleDate,
            ).format('DD-MM-YYYY')}`}</RN.Text>
            <RN.View style={{justifyContent: 'center'}}>
              <RN.Image
